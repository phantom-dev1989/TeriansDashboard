...
jQuery.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		// we once tried to use readyState "interactive" here, but it caused issue like the one
...
;(function() {
  ...
  /** Used to prefix keys to avoid issue with `__proto__` and properties on `Object.prototype` */
  ...
  function compareAscending(a, b) {
    ...
    // http://code.google.com/p/v8/issue/detail?id=90
    if (ac !== bc) {
  ...
  function runInContext(context) {
    ...
    function isObject(value) {
      ...
      // and avoid a V8 bug
      // http://code.google.com/p/v8/issue/detail?id=2291
    ...
    function template(text, data, options) {
      ...
      // avoid missing dependency when `iteratorTemplate` is not defined
...
joint.dia.Cell = Backbone.Model.extend({
    ...
    prop: function(props, value, opt) {
		...
		// Pure integer keys will cause issue and are therefore not allowed.
